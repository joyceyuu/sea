...
typecheck-xml:
	mypy --junit-xml=test_output/mypy/results.xml ./Sea

lint:
	py.test --flake8 ./Sea -p no:regtest --cache-clear

lint-xml:
	py.test --junit-xml=test_output/flake8/results.xml --flake8 ./Sea -p no:regtest --cache-clear

test:
	py.test --cov=./Sea --cache-clear --cov-fail-under=80 .

test-xml:
	py.test --junit-xml=test_output/pytest/results.xml --cov=./Sea --cov-report=html:test_output/coverage --cache-clear --cov-fail-under=80 .
